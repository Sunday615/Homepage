<script setup lang="ts"></script>

<template>
    <div class="logo-box">

        <div class="title">
                  <div data-aos="zoom-out-up" data-aos-duration="1000">
            <h1>ສະມາຊິກລະບົບຊຳລະຂ້າມແດນໃນຮູບແບບ QR Code ລະຫວ່າງປະເທດ</h1></div>
        </div>
        <div class="logo">
            <div class="row1">
                <div class="boxlogo1"><img  style="width: 170px; height:110px;"  src="../../../assets/Logomember- retangle/bcelretangle.png" alt=""></div>
                <div class="boxlogo2">
                    <img style="width: 195px; " src="../../../assets//Logomember- retangle/ldbwithborder.png" alt="">
                </div>
                <div class="boxlogo3"><img style="width: 140px; height:140px" src="../../../assets/Logomember- retangle/apbpng.png" alt=""></div>
                <div class="boxlogo4">
                    <img style="width: 190px;" src="../../../assets/Logomember- retangle/jdb.png" alt="">
                </div>
                <div class="boxlogo5">
                    <img style="width: 180px; height:180px" src="../../../assets/Logomember- retangle/MARU.jpg" alt="">
                </div>
                <div class="boxlogo6">
                    <img style="width: 190px;"  src="../../../assets/Logomember- retangle/LVB.png" alt="">
                </div>
            </div>
            <div class="row1">
              
                <div class="boxlogo9">
                    <img style="width:190px ; padding-bottom: 30px;" src="../../../assets/Logomember- retangle/y-nghia-sau-sac-dang-sau-thiet-ke-logo-vietinbank-0.png" alt="">
                </div>
                <div class="boxlogo10">
                    <img style="width:190px" src="../../../assets/Logomember- retangle/IB.png" alt="">
                </div>
                <div class="boxlogo11">
                    <img style="width:190px" src="../../../assets/Logomember- retangle/logo-share.jpg" alt="">
                </div>
                <div class="boxlogo12">
                    <img style="width:190px" src="../../../assets/Logomember- retangle/BIC.png" alt="">
                    
                </div>
                  <div class="boxlogo13">
                    <img style="width:160px" src="/Logomember/sacom.png" alt="">
                </div>
                <div class="boxlogo14">
                    <img style="width:170px" src="../../../assets/Logomember- retangle/Stbretangle.png" alt="">
                </div>
            </div>
            <div class="row1">
       

                <div class="boxlogo15">
                    <img style="width:150px" src="../../../assets/Logomember- retangle/kbank.jpg" alt="">
                </div>

                <div class="boxlogo16">
                    <img style="width:180px" src="../../../assets/Logomember- retangle/psvb.PNG" alt="">
                </div>
                <div class="boxlogo16">
                    <img style="width:180px" src="../../../assets/Logomember- retangle/mbbgblue.jpg" alt="">
                </div>
              
            </div>
           
        </div>
    </div>
</template>


<style scoped>
.boxlogo1, .boxlogo2, .boxlogo3, .boxlogo4, .boxlogo5, .boxlogo6 , .boxlogo7, .boxlogo8, .boxlogo9, .boxlogo10, .boxlogo11, .boxlogo12, .boxlogo13, .boxlogo14, .boxlogo15, .boxlogo16{
    width: 210px;
    height: 100%;
    background-color: #fff;
    display: inline-block;
    margin-right: 5px;
    margin-left: 3px;
    display: flex;
    justify-content: center;
    align-items: center;

   
   
}
.row1{
    width: 80%;
    height: 200px;

    margin: 0 auto;
    display: flex;
    justify-content: center;
    margin-top: 10px;
}
#bg{
    position: absolute;
    z-index: -1;
    width: 100%;
    height: 100%;
 
}
.title h1{
    padding-top: 50px;
    width: 70%;
    margin: 0 auto;

    font-size: 60px;
    font-family: "Noto Sans Lao", sans-serif;
    font-weight: bold;

    text-align: center;
}
.logo{
    
    width: 100%;
    height: 75%;
    padding-top: 150px;
    
    
}
.title{
    width: 100%;
    height: 15%;

    padding-top: 40px;
}
.logo-box{
    width: 100%;
    height: 1200px;
    background-image: url("../../../assets/footer/footerasset.png");
  
}

</style>